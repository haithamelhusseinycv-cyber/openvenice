...
export function createFaceFusionPlugin(connector: FaceFusionConnector): AgentPluginDefinition {
  return {
    manifest: {
      ...
      name: 'FaceFusion',
      version: 'builtin',
      description: 'Native Android face detection, swap, restoration, and enhancement through the signature-protected FaceFusion bridge. Minimum model packs can be downloaded on-device with facefusion.ensure_models.',
      capabilities: ['face-detection', 'face-swap', 'face-enhance', 'frame-enhance', 'model-download'],
      permissions: ['network', 'local-files', 'local-app-control'],
