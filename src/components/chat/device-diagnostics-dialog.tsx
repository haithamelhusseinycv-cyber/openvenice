...
export function DeviceDiagnosticsDialog({ open, onClose }: DeviceDiagnosticsDialogProps) {
  ...
  const testSpeaker = async () => {
    ...
    try {
      const text = locale === 'ar-EG'
        ...
        : 'Hi, I am Nour. English voice output is working.'
  ...
  const copyReport = async () => {
    const report = [
      `OpenVenice device acceptance · ${new Date().toISOString()}`,
      ...
      <section className="relative flex max-h-[88dvh] w-full max-w-full min-w-0 flex-col overflow-hidden rounded-t-2xl border border-white/[0.1] bg-[#111116] shadow-2xl sm:max-w-xl sm:rounded-2xl">
        <div className="flex items-center gap-2 border-b border-white/[0.07] px-4 py-3">
          <div className="min-w-0 flex-1">
            <h2 className="text-[17px] font-semibold text-white">Device acceptance</h2>
            <p className="text-[12px] text-white/40">OnePlus / Android readiness checks</p>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-4">
          <div className="mb-3 grid grid-cols-3 gap-2">
            ...
            <div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.05] px-3 py-2 text-center"><div className="text-[18px] font-semibold text-amber-100">{summary.warn}</div><div className="text-[10px] uppercase tracking-wide text-white/35">Check</div></div>
          ...
          <p className="mt-3 px-1 text-[10.5px] leading-relaxed text-white/30">Checks are local and do not expose API keys. Microphone and speaker tests only run when you tap them.</p>
        </div>
      </section>
    </div>
  )
