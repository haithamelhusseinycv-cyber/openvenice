...
const modelTypeMap: Partial<Record<string, 'text' | 'image'>> = {
  chat: 'text',
  image: 'image',
}
...
const tabSubtitles: Record<string, string> = {
  chat: 'Allowlisted Venice chat models',
  image: 'Generate and edit still images',
  workflows: 'Chain Chat and Image Gen visually',
...
export function Header({ onOpenApiKey, onOpenMobileSidebar }: Props) {
  ...
  const imageSubTab = useImageWorkspace((state) => state.imageSubTab)
  const apiKey = useAuthStore((state) => state.apiKey)

  const modelType = modelTypeMap[activeTab]
  const showModelSelector = !!modelType && !(activeTab === 'image' && imageSubTab === 'tools')
  const { data: models } = useModels(modelType, showModelSelector)
  const currentModel = showModelSelector ? (selectedModels[activeTab] || models?.[0]?.id || '') : ''
  const modelOptions = showModelSelector
    ? (models?.map((model) => ({ value: model.id, label: model.model_spec?.name || model.id })) ?? [])
    : []
  ...
  return (
    <header className="flex items-center gap-3 h-14 px-3 border-b border-white/[0.05] bg-[#0a0a0c] shrink-0">
      ...
      <div className="flex flex-col min-w-0">
        <span className="text-[14px] font-semibold text-white/95 leading-none">{tabLabels[activeTab] || 'OpenVenice'}</span>
      ...
      {showModelSelector && (
        <>
          ...
          <Select
            value={currentModel}
            onChange={(value) => setSelectedModel(activeTab, value)}
