...
/** Complete Models browser with runtime selection for supported Android processors. */
public class ModelCatalogActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText("Face Fusion — Complete Models v2");
        ...
        note.setText("Supported processing chain: INSwapper → optional CodeFormer/GFPGAN → optional Real-ESRGAN/UltraSharp. Other catalog models remain downloadable for future adapters. Models are loaded from local app storage and processing stays on-device.");
        note.setTextSize(12);
    ...
    private void configureUseButton(MaterialButton use, ModelCatalog.ModelPack pack, String mode) {
        ...
        if ("swap".equals(mode)) {
            key = "swapper_model";
            selected = prefs.getString(key, "inswapper_128.onnx");
            use.setText(file.equals(selected) ? "Selected" : "Use");
            use.setEnabled(isDownloaded(pack) && !file.equals(selected));
            use.setOnClickListener(v -> {
                prefs.edit().putString(key, file).apply();
                Toast.makeText(this, "Swapper selected. Restart Face Fusion to reload the swap engine.", Toast.LENGTH_LONG).show();
                rebuildList();
            });
            return;
        }
    ...
    private String supportedMode(ModelCatalog.ModelPack pack) {
        if (pack.selectableSwapper) return "swap";
        if ("codeformer".equals(pack.id) || "gfpgan_1.3".equals(pack.id) || "gfpgan_1.4".equals(pack.id)) return "face";
        if ("real_esrgan_x2".equals(pack.id) || "real_esrgan_x2_fp16".equals(pack.id)
                || "real_esrgan_x4".equals(pack.id) || "real_esrgan_x4_fp16".equals(pack.id)
                || "ultra_sharp_x4".equals(pack.id) || "ultra_sharp_2_x4".equals(pack.id)) return "frame";
        return null;
    }

    private String supportedBadge(ModelCatalog.ModelPack pack) {
        String mode = supportedMode(pack);
        if ("swap".equals(mode)) return "  ✓ SWAP";
        if ("face".equals(mode)) return "  ✓ FACE";
    ...
    private HttpURLConnection open(String initial, long offset) throws Exception {
        ...
        for (int redirects = 0; redirects < 10; redirects++) {
            ...
            c.setRequestProperty("User-Agent", "AndroidFaceFusion/CompleteModelsV2");
