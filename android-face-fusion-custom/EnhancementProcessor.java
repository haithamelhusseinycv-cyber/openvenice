...
/**
 * Optional post-processing pipeline for the Complete Models Android build.
 *
 * Supported face enhancers:
 *   CodeFormer, GFPGAN 1.3, GFPGAN 1.4
 * Supported frame enhancers:
 *   Real-ESRGAN x2/x4 (FP32 + FP16), UltraSharp x4, UltraSharp 2 x4
 *
 * Models are never bundled here. They are loaded from the app's private files
 * directory after the user downloads them from Complete Models.
 */
public final class EnhancementProcessor {
    private static final String TAG = "EnhancementProcessor";
    ...
    public Bitmap apply(Bitmap input) {
        ...
        if (faceModel != null && !"none".equals(faceModel)) {
            File model = new File(context.getFilesDir(), faceModel);
            if (model.exists() && model.length() > 1024 * 1024L && isSupportedFaceModel(faceModel)) {
                try {
                    Bitmap enhanced = enhanceFaces(current, model);
                    if (enhanced != current) {
        ...
        if (frameModel != null && !"none".equals(frameModel)) {
            File model = new File(context.getFilesDir(), frameModel);
            FrameSpec spec = getFrameSpec(frameModel);
            if (model.exists() && model.length() > 1024 * 1024L && spec != null) {
                try {
                    Bitmap enhanced = enhanceFrame(current, model, spec);
                    if (enhanced != current) {
                        if (current != input) current.recycle();
                        current = enhanced;
                    }
                } catch (Throwable t) {
                    Log.e(TAG, "Frame enhancement failed; keeping previous result", t);
                }
            }
        }
    ...
    private boolean isSupportedFaceModel(String file) {
        return "codeformer.onnx".equals(file)
                || "gfpgan_1.3.onnx".equals(file)
                || "gfpgan_1.4.onnx".equals(file);
    }

    private Bitmap enhanceFaces(Bitmap input, File modelFile) throws Exception {
        ...
        try {
            ...
            for (FaceDetector.Face face : faces) {
                Bitmap aligned = ImageUtils.alignFace(result, face.landmarks, FACE_SIZE);
                Bitmap restored = runFaceEnhancer(session, aligned);
                Bitmap blended = ImageUtils.blendFaces(result, restored, face.landmarks, FACE_SIZE);
    ...
    private Bitmap runFaceEnhancer(OrtSession session, Bitmap face) throws Exception {
        ...
        OnnxTensor imageTensor = createTensor(data, new long[]{1, 3, FACE_SIZE, FACE_SIZE}, imageInfo.type);
    ...
    private Bitmap enhanceFrame(Bitmap source, File modelFile, FrameSpec spec) throws Exception {
        // MainActivity caps input images to 1024 px. Keep an additional guard so x4
        // processing remains reasonable on mobile even if that changes upstream.
        Bitmap input = source;
        ...
        try {
            ...
            int padBottom = padTop + tileInner - (input.getHeight() + 2 * spec.pad) % tileInner;
            int padRight = padTop + tileInner - (input.getWidth() + 2 * spec.pad) % tileInner;
            ...
            int outPaddedW = paddedW * spec.scale;
            int outPaddedH = paddedH * spec.scale;
            Bitmap merged = Bitmap.createBitmap(outPaddedW, outPaddedH, Bitmap.Config.ARGB_8888);
            ...
            // Match FaceFusion's default 80% enhancer blend.
    ...
    private Bitmap tensorToBitmap(OnnxTensor tensor, boolean minusOneToOne) throws Exception {
        ...
        if (shape.length != 4 || shape[0] != 1 || shape[1] < 3) {
            throw new Exception("Unsupported enhancer output shape");
        }
        int h = (int) shape[2];
        int w = (int) shape[3];
        int plane = h * w;
